...
```{r}
testthat::expect_equal(unique(t$origin), origins_values)
```

The correct answers is `r origins_values`.
...
```{r}
testthat::expect_equal(unique(t$transplant), transplanted_values)
```
